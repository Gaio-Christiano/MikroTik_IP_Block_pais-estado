...
# Expressão regular para extrair Tipo, IP, Data, Hora e Timeout (Timeout é opcional)
pattern = r"(PORTASCAN|API_PORTASCAN|PORTASCAN1|bogons|Blocked|blocked-scanners)\s+([\d\.]+)\s+(\d{4}-\d{2}-\d{2})?\s*([\d:]+)?\s*([\d\w]+)?"

# Listas para armazenar os dados e linhas inválidas
dados = []
linhas_invalidas = []

# Ler o arquivo e validar as linhas
with open(file_path, "r") as arquivo:
    for idx, linha in enumerate(arquivo, start=1):
...
for entrada in dados:
    ip = entrada["IP"]
    print(f"Processando IP {entrada['Índice']}/{len(dados)}: {ip}")
...
arquivo_saida = f"04_Relatorio_Completo_{hora_atual}.xlsx"
